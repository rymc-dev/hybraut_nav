...
from automaton.core_interfaces import IOSpec
...
class MockReset(ResetInterface):
    ...
    _init_input_spec = [
        IOSpec.create_io_spec(name='i', type=float)
    ]

    _state_input_spec = [
        IOSpec.create_io_spec(name='x', type=float)
    ]

    _evaluation_output_type = [
        IOSpec.create_io_spec(name='velocity_state', type=float),
        IOSpec.create_io_spec(name='yaw_rate_state', type=float)
    ]
