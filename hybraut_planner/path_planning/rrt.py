#!/usr/bin/env python3
"""
RRT (Rapidly-exploring Random Tree) implementation for path planning.
"""
import math
import random
...
class RRT(Planner):
            ...
            if self.is_collision_free(nearest_node, new_node):
                ...
                if self.distance(new_node, Node(end.x, end.y)) <= self.step_size:
                    ...
                    if self.is_collision_free(new_node, goal_node):
                        ...
                        print(f"RRT found path after {iteration + 1} iterations")
    ...
    def extract_path(self, goal_node: Node) -> Tuple[Path, float]:
        ...
        for i in range(len(path_nodes) - 1):
            total_cost += self.distance(path_nodes[i], path_nodes[i + 1])
...
def main():
    ...
    # Run RRT
    rrt = RRT(obstacle_threshold=200, max_iter=10000, step_size=2, goal_sample_rate=0.1)
    path, cost = rrt.plan_path(g, start, goal)
