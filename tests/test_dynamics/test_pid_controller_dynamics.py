...
import pytest
import math
from hybraut_lifecycle.dynamics import PIDControllerDynamics, DynamicsABC
...
class TestPIDControllerDynamics:
    ...
    def test_initialization_and_specs(self, dynamics):
        ...
        assert (
            dynamics.dynamic_output_spec_description() == "outputs for automaton spec"
        )
    ...
    def test_string_representations(self, dynamics):
        ...
        assert (
            repr(dynamics)
            == "PIDControllerDynamics(output_type=AutomatonControlOutputs, initialized=True)"
        )
        assert (
            str(dynamics)
            == "Dynamics Function: PIDControllerDynamics (Output: AutomatonControlOutputs)"
        )
    ...
    def test_comprehensive_velocity_and_yaw(self, dynamics, state_kwargs):
        ...
        assert (
            velocity_range <= max_reasonable_oscillation
        ), f"Velocity oscillation too large: {velocity_range} > {max_reasonable_oscillation}"

        # Should not exceed max_velocity (check all velocities)
        assert all(
            v <= dynamics.max_velocity for v in velocities
        ), "Velocity exceeded maximum"
        ...
        assert (
            max_yaw_rate < 0.1
        ), f"Yaw rate too large for straight motion: {max_yaw_rate}"
        ...
        if len(early_stable) > 0 and len(late_stable) > 0:
            ...
            assert (
                late_avg_error <= early_avg_error * 1.1
            ), "System appears to be diverging from target"
        ...
        assert (
            final_avg_error <= max_acceptable_error
        ), f"Final error too large: {final_avg_error} > {max_acceptable_error}"
        ...
        assert (
            stable_velocity_std <= max_acceptable_std
        ), f"Velocity too unstable: std={stable_velocity_std} > {max_acceptable_std}"
    ...
    def test_pid_controller_basic_functionality(self, dynamics, state_kwargs):
        ...
        assert all(
            isinstance(v, (int, float)) for v in velocities
        ), "Velocities should be numeric"
        assert all(
            isinstance(y, (int, float)) for y in yaw_rates
        ), "Yaw rates should be numeric"

        # Bounds checking
        assert all(
            0 <= v <= dynamics.max_velocity for v in velocities
        ), "Velocity out of bounds"

        # Controller should produce reasonable outputs
        assert not all(
            v == velocities[0] for v in velocities
        ), "Controller appears inactive"

        # For straight motion, yaw rates should be small
        assert all(
            abs(y) < 0.5 for y in yaw_rates
        ), "Excessive yaw rate for straight motion"
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                {"agent_state": "invalid", "waypoints_state": ROSWaypointsState()},
                TypeError,
            ),
    ...
    def test_invalid_state_inputs(
        self, dynamics, state_kwargs_input, exception, far_waypoint
    ):
        """
    ...
    def test_reset_and_diagnostics(self, dynamics, state_kwargs):
        ...
        assert math.isclose(
            diag_after["avg_dt"], 1.0 / dynamics.control_frequency, rel_tol=1e-3
        )
