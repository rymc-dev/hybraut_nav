...
from hybraut_interfaces.msg import GuardEvaluationMSG, InvariantEvaluationsMSG
...
@dataclass
class GuardBus(WrapperInterface):
    """ 
    GuardBus
    Guard bus is utilized for uploading guard events
    """

    
    _topic: str = field(default="/automaton/guard_event", init=False)
...
class GuardWrapper(WrapperInterface):
    ...
    @classmethod
    def load_guard_from_amdl(cls, guard_name: str, guard_dict: Dict[str, Any]) -> 'GuardWrapper':
...
class GuardRegistry(ComponentRegistry['GuardWrapper']):
    ...
    def evaluate_guard_by_name(self, guard_name: str, ctx: EvaluationContext) -> GuardEvaluationMSG:
    ...
    def evaluate_guards_by_name(self, guard_names: List[str], ctx: EvaluationContext) -> List[GuardEvaluationMSG]:
    ...
    def register(cls, guard_dict: Dict[str, Any]) -> Dict[str, GuardWrapper]:
        ...
        for name, conf in guard_dict.items():
            try: 
                ...
                component = component_cls.load_guard_from_amdl(guard_name=name, guard_dict=conf)
    ...
    @classmethod
    def load_guard_registry_from_amdl(cls,guard_dict: Dict[str, Any]) -> 'GuardRegistry':
...
if __name__ == '__main__':
    ...
    guard_registry:GuardRegistry = GuardRegistry.load_guard_registry_from_amdl(guard_dict)
    guard_evaluations: List[GuardEvaluationMSG] = guard_registry.evaluate_guards_by_name(guard_names=guard_dict.keys(), ctx=evaluation_context)
