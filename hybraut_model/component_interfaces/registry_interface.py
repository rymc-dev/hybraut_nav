...
class ComponentRegistry(Registry[T]):
    ...
    def add_component(self, name: str, component: T) -> None:
        ...
        if name in self._components:
            error_msg = (
                f"{self._component_type_name} '{name}' already exists in registry"
            )
            self._logger.error(error_msg)
        ...
        self._logger.info(
            f"Added {self._component_type_name.lower()} '{name}' to registry"
        )
    ...
    def remove_component(self, name: str, node: Optional[Node] = None) -> None:
        ...
        if component.is_active:
            ...
            try:
                ...
                self._logger.debug(
                    f"Deactivated {self._component_type_name.lower()} '{name}' before removal"
                )
            except Exception as e:
                self._logger.warning(
                    f"Failed to deactivate {self._component_type_name.lower()} '{name}' during removal: {e}"
                )

        del self._components[name]
        self._logger.info(
            f"Removed {self._component_type_name.lower()} '{name}' from registry"
        )
    ...
    def activate_components(
        ...
        self._logger.info(
            f"Activating {len(self._components)} {self._component_type_name.lower()}s"
        )
        ...
        try:
            for component_name, component in self._components.items():
                self._logger.debug(
                    f"Activating {self._component_type_name.lower()}: {component_name}"
                )
                component.activate()
            ...
            self._logger.info(
                f"Successfully activated all {self._component_type_name.lower()}s: {activated_components}"
            )

        except Exception as e:
            # Rollback: deactivate any components that were successfully activated
            self._logger.error(
                f"Failed to activate {self._component_type_name.lower()}s, rolling back: {e}"
            )
            for component_name in activated_components:
                try:
                    self._logger.debug(
                        f"Rolling back activation for {self._component_type_name.lower()}: {component_name}"
                    )
                    self._components[component_name].deactivate(node)
                except Exception as rollback_error:
                    self._logger.error(
                        f"Rollback failed for {self._component_type_name.lower()} '{component_name}': {rollback_error}"
                    )
            raise
    ...
    def deactivate_components(self, node: Node) -> None:
        ...
        self._logger.info(
            f"Deactivating {len(self._components)} {self._component_type_name.lower()}s"
        )
        ...
        for component_name, component in self._components.items():
            try:
                self._logger.debug(
                    f"Deactivating {self._component_type_name.lower()}: {component_name}"
                )
        ...
        if deactivation_errors:
            self._logger.warning(
                f"Some {self._component_type_name.lower()}s failed to deactivate properly: {len(deactivation_errors)} errors"
            )
        else:
            self._logger.info(
                f"Successfully deactivated all {self._component_type_name.lower()}s"
            )
    ...
    def validate_dependencies(
        ...
        """
        missing_components = [
            name for name in required_components if name not in self._components
        ]
        if missing_components:
        ...
        if required_types and len(required_types) == len(required_components):
            type_mismatches = []
            for component_name, expected_type in zip(
                required_components, required_types
            ):
            ...
            if type_mismatches:
                error_msg = (
                    f"{self._component_type_name} type mismatches: {type_mismatches}"
                )
                self._logger.error(error_msg)
                raise ValueError(error_msg)

        self._logger.debug(
            f"{self._component_type_name} dependencies validated: {required_components}"
        )
        return True
