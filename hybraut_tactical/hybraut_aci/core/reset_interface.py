...
class ResetInterface(HybridComponentInterface):
    ...
    def _validate_reset_targets(self) -> None:
        ...
        for i, target in enumerate(self._reset_targets_spec):
            ...
            if not hasattr(target, "name") or not isinstance(target.name, str):
                raise TypeError(
                    f"_reset_targets_spec[{i}] must have a valid 'name' attribute as string"
                )

            if not hasattr(target, "type") or not isinstance(target.type, type):
                raise TypeError(
                    f"_reset_targets_spec[{i}] must have a valid 'type' attribute as type"
                )
        ...
        for name, value in reset_output.items():
            if name in target_types:
                ...
                if not isinstance(value, expected_type):
                    raise TypeError(
                        f"Reset output '{name}' expected type {expected_type.__name__}, got {type(value).__name__}"
                    )
        ...
        info.update(
            {
                ...
                "reset_target_spec_types": [
                    t.__name__ for t in self.get_reset_target_spec_types()
                ],
                "reset_specific_info": {
    ...
    def __repr__(self) -> str:
        ...
        target_names = (
            [target.name for target in self._reset_targets_spec]
    ...
    def __str__(self) -> str:
        ...
        target_count = (
            len(self._reset_targets_spec) if hasattr(self, "_reset_targets_spec") else 0
        )
