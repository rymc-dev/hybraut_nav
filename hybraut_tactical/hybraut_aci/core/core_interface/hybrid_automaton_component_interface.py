...
class HybridComponentInterface(ABC):
    ...
    def _validate_initialization(self, **init_kwargs) -> None:
        ...
        for expected_init_input in self._init_input_spec:
            if expected_init_input.name not in init_kwargs:
                raise KeyError(
                    f"Missing required initialization parameter: '{expected_init_input.name}'"
                )
    ...
    def _validate_states(self, **state_kwargs) -> None:
        ...
        if not self.is_initialized:
            raise RuntimeError(
                f"{self.__class__.__name__} component is not initialized"
            )

        for expected_state_input in self._state_input_spec:
            if expected_state_input.name not in state_kwargs:
                raise KeyError(
                    f"Missing required state input: '{expected_state_input.name}'"
                )
    ...
    def get_component_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "init_input_spec_types": [
                t.__name__ for t in self.get_init_input_spec_types()
            ],
            "state_input_spec_names": self.get_state_input_spec_names(),
            "state_input_spec_types": [
                t.__name__ for t in self.get_state_input_spec_types()
            ],
        }
