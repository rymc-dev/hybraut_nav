...
class DynamicsBus:
    ...
    def publish_dynamic(self, msg: Any):
        if not isinstance(msg, self._dynamic_publisher.msg_type):
            raise RuntimeError(f"attempted to publish invalid msg type: {type(msg)}, expected {self._dynamic_type}")
    ...
    @classmethod
    def create_dynamic_bus(cls, node: Node, dynamic_name: str, dynamic_topic: str, dynamic_type: Type, dynamic_cb_group: CallbackGroup = ReentrantCallbackGroup(), dynamic_qos: QoSProfile = qos_profile_system_default) -> 'DynamicsBus':
...
class DynamicsHub:
    ...
    def __init__(self, node: Node, dynamics_registry: DynamicsRegistry, cb_group: CallbackGroup = ReentrantCallbackGroup(), qos: QoSProfile = qos_profile_system_default):
        
        self._busses: Dict[str, DynamicsBus] = {}

        dynamic_components = dynamics_registry.get_components_by_names(dynamics_registry.get_component_names())
