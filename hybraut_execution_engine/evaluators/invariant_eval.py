...
class InvariantEvaluator:
    ...
    def __call__(self, *, current_mode: int) -> None:
        ...
        with self.lock:
            self._evaluate_invariants(current_mode=current_mode)
...
def main() -> None:
    ...
    evaluator = InvariantEvaluator(node=node, automaton=automaton)
    ...
    node.create_timer(
        0.1,
        lambda: evaluator._evaluate_invariants(current_mode=current_mode[0]),
