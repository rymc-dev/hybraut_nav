...
class PIDControllerDynamics(DynamicsInterface):
    ...
    _init_input_spec = [
        ...
        IOSpec.create_io_spec(
            name="derivative_filter_alpha", type=float
    ...
    def _validate_initialization(self, **init_kwargs):
        ...
        for param_name, value in init_kwargs.items():
            if param_name in [
                ...
                if value < 0.0:
                    raise ValueError(
                        f"init param '{param_name}' must be non-negative, got {value}"
                    )
            elif param_name in [
                "control_frequency",
                "max_velocity",
                ...
                if value <= 0.0:
                    raise ValueError(
                        f"init param '{param_name}' must be positive, got {value}"
                    )
    ...
    def _calculate_distance_to_waypoint(
        self, agent_state: ROSAgentState, waypoint: ROSWaypoint
    ) -> float:
    ...
    def _scale_velocity_for_approach(
        self, base_velocity: float, distance: float
    ) -> float:
        """Scale velocity down when approaching waypoint."""
        if distance < self._min_approach_distance:
            scale_factor = max(
                self._approach_velocity_scale, distance / self._min_approach_distance
            )
    ...
    def _filtered_derivative(
        ...
        current_error: float,
        prev_error: float,
        prev_filtered_derivative: float,
        dt: float,
        alpha: float,
    ) -> float:
    ...
    def _analyze_velocity_trend(self) -> float:
        ...
        if len(self._velocity_trend_buffer) >= 3:
            ...
            trend = (recent_velocities[-1] - recent_velocities[0]) / len(
                recent_velocities
            )
            return trend
    ...
    def _evaluate(self, **state_kwargs):
        ...
        self._filtered_heading_derivative = self._filtered_derivative(
            heading_error,
            self._prev_heading_error,
            self._filtered_heading_derivative,
            dt,
            alpha,
        )
        ...
        # Smooth output
        target_yaw_rate = self._smooth_output(
            target_yaw_rate, self._yaw_rate_output_history
        )
        ...
        self._filtered_velocity_derivative = self._filtered_derivative(
            velocity_error,
            self._prev_velocity_error,
            self._filtered_velocity_derivative,
            dt,
            alpha,
        )
        ...
        final_velocity = self._smooth_output(
            final_velocity, self._velocity_output_history
        )
    ...
    def get_pid_diagnostics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "waypoints_state_buffer_size": len(
                self._state_buffer.get("waypoints_state", [])
            ),
    ...
    def get_buffer_analysis(self) -> Dict[str, Any]:
        ...
        velocity_variance = sum((v - avg_velocity) ** 2 for v in velocities) / len(
            velocities
        )

        # Analyze position changes
        positions = [
            (state.pose.position.x, state.pose.position.y) for state in agent_states
        ]
        if len(positions) >= 2:
            total_distance = sum(
...
if __name__ == "__main__":
    ...
    for i in range(10000):
        ...
        if i % 50 == 0:
            print(
                f"Step {i:3d}: pos=({vehicle_x:6.2f},{vehicle_y:6.2f}), "
