...
class TestGuardsNode(unittest.TestCase):
    ...
    def test_start_guards_eval_srv(self):
        ...
        expected_topic_names_and_types = {
            '/hybrid_automaton/guards': 'colav_interfaces/msg/AgentUpdate',
            '/hybrid_automaton/mode': 'colav_interfaces/msg/GuardsStatus',
            '/hybrid_automaton/waypoints': 'std_msgs/msg/String',
            '/agent_update': 'colav_interfaces/msg/Waypoints',
        ...
        # Loop through the list of topic names and types
        for topic_name_and_types in topic_names_and_types:
            topic_name = topic_name_and_types[0]
            topic_types = topic_name_and_types[1]

            # Check if the topic name is in the expected dictionary
            if topic_name in expected_topic_names_and_types:
                expected_type = expected_topic_names_and_types[topic_name]
                
                # Since topic_types is a list, we need to compare the first element in the list
                if expected_type in topic_types:
                    # If the type matches, remove the item from the expected dictionary
                    del expected_topic_names_and_types[topic_name]

        print('after: ')
        print(expected_topic_names_and_types)

        # Check if there are any remaining topics in the expected dictionary
