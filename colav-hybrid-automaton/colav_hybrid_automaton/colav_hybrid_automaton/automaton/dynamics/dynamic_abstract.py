"""
Hybrid Automaton Dynamic Abstract Base Class

This abstract class defines the interface for dynamic functions in the hybrid automaton framework.
Dynamic functions are executed during mode execution to modify continuous state variables.

All dynamic implementations must inherit from this class and implement the required methods.
...
Example Usage:
    class SimplePIDController(HybridAutomatonReset):
        def __init__(self, min_waypoints: int = 1, dt:float, ):
            do something
            
        def __call__(self, agent_state: AgentState, waypoints_state: WaypointsState) -> Tuple[HybridAutomatonDynamics]:
...
class HybridAutomatonDynamic(ABC):
    """
    Abstract base class for hybrid automaton dynamic functions.
    
    Dynamic functions are executed during mode executions to modify continuous
    ...
    @abstractmethod
    def __call__(self, *state_inputs) -> Tuple[Any, ...]:
        """
        Execute the dynamic function with current state inputs.
        
        This method is executed during mode execution as a callback,
        every callback we will update the dynamics based on the previous states
    ...
    def __str__(self) -> str:
        ...
        return f"Dynamic Function: {self.__class__.__name__}"
